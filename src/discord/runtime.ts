...
export class DiscordRuntime implements IdentityProvider, Effects {
  ...
  private async interaction(i:Interaction):Promise<void> {
    ...
    try{
      ...
      buttons.push(new ButtonBuilder().setURL(`${this.config.PANEL_URL}/#${mapping[page]??"dashboard"}`).setLabel("Abrir painel completo").setStyle(ButtonStyle.Link));
